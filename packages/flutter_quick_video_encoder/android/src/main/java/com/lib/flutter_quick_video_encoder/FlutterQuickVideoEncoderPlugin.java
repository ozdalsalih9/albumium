    ...
    MethodChannel.MethodCallHandler
{
            ...
            switch (call.method) {
                ...
                case "setup":
                {
                    ...
                    if (width != 0 && height != 0) {
                        ...
                        // Video encoder. It is created before the format so its
                        // capabilities can be queried. Without an explicit bitrate
                        // mode most hardware encoders run VBR and treat KEY_BIT_RATE
                        // as a ceiling, undershooting badly on low-motion content
                        // (a 10 Mbps 1080p request produced ~2 Mbps). Ask for CBR
                        // and High profile where supported; if the encoder rejects
                        // them, fall back to the original plain format.
                        mVideoEncoder = MediaCodec.createEncoderByType("video/avc");
                        MediaCodecInfo.CodecCapabilities videoCaps =
                            mVideoEncoder.getCodecInfo().getCapabilitiesForType("video/avc");
                        MediaFormat videoFormat =
                            createVideoFormat(width, height, fps, videoBitrate, colorFormat);
                        applyQualityHints(videoFormat, videoCaps);
                        try {
                            Log.i(TAG, "calling mVideoEncoder.configure() " + videoFormat);
                            mVideoEncoder.configure(videoFormat, null, null, MediaCodec.CONFIGURE_FLAG_ENCODE);
                        } catch (Exception e) {
                            Log.w(TAG, "encoder rejected CBR/High profile, using defaults: " + e);
    ...
    private static MediaFormat createVideoFormat(
            int width, int height, int fps, int bitrate, int colorFormat) {
    ...
    // Constant bitrate makes the encoder actually spend the requested bits, and
    // High profile compresses better than the Baseline default.
    //
    // CBR is requested even when the capabilities do not list it: vendor
    // media_codecs.xml files often omit the "bitrate-modes" feature (seen on
    // OMX.qcom.video.encoder.avc, Galaxy S20 FE) although the component honours
    // CBR, and Android then reports VBR only. An encoder that truly rejects it
    // fails configure(), and the caller falls back to the plain format.
    private static void applyQualityHints(
            MediaFormat format, MediaCodecInfo.CodecCapabilities caps) {
        format.setInteger(MediaFormat.KEY_BITRATE_MODE,
            MediaCodecInfo.EncoderCapabilities.BITRATE_MODE_CBR);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && caps.profileLevels != null) {
            int highLevel = 0;
            for (MediaCodecInfo.CodecProfileLevel profileLevel : caps.profileLevels) {
                if (profileLevel.profile == MediaCodecInfo.CodecProfileLevel.AVCProfileHigh
                        && profileLevel.level > highLevel) {
                    highLevel = profileLevel.level;
                }
            }
            if (highLevel > 0) {
                format.setInteger(MediaFormat.KEY_PROFILE,
                    MediaCodecInfo.CodecProfileLevel.AVCProfileHigh);
                format.setInteger(MediaFormat.KEY_LEVEL, highLevel);
            }
        }
    }
