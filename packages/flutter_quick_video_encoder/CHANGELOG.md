## 1.7.2+albumium.1
* Albumium patch, Android: request `BITRATE_MODE_CBR` and AVC High profile when the
  encoder supports them, falling back to the original format if configure fails.
  Upstream sets no bitrate mode, so hardware encoders ran VBR and undershot the
  requested bitrate on low-motion content.
* CBR is requested even when capabilities omit it; vendor media_codecs.xml files
  (e.g. Qualcomm OMX on Galaxy S20 FE) often do not declare `bitrate-modes`.
